...
class PD:
    def __init__(
        ...
        logger_config["handlers"]["filehand"]["filename"] = (
            my_repo_folder
            + "\\logs\\"
            + logger_config["handlers"]["filehand"]["filename"]
        )
                ...
                if auto == False:
                    ...
                    while True:
                        ...
                        if (
                            isinstance(ins, int)
                            and ins >= 0
                            and ins <= len(self.search_results)
                        ):
                            break
                        if ins == "end":
                            exit()
                            ...
                            self.logger.warning(
                                f"Enter a valid number(You have {10 - count} more trials)"
                            )
                        count += 1
                    if count <= 10:
            ...
            if r.status_code <= 400:
                ...
                while episodes_json["next_page_url"] != None:
                    ...
                    r: requests.Response = requests.get(
                        url + page_num, headers=self.headers
                    )
    ...
    def get_kwix(self, anime, episode):
        ...
        try:
            ...
            if r.status_code <= 400:
                ...
                resp_headers = requests.post(
                    post_url, data=payload, headers=head, allow_redirects=False
                )
                # print(resp_headers)
                # print(resp_headers)
                try:
    ...
    def download(self):
        ...
        self.logger.info(
            f"Getting episodes between {first} and {last} episodes for {list(self.search_result.keys())[0]}"
        )
        ...
        if isinstance(self.search_result, dict):
            ...
            e_range: list[dict] = [
                n for n in episodes_search[1] if first <= int(n["episode"]) <= last
            ]
            t1: float = time.perf_counter()
            for x in e_range:
...
def main():
    t1 = time.perf_counter()
    parse = argparse.ArgumentParser(description="A script to get anime download links")
    parse.add_argument(
        "--should_append",
        "-sa",
        type=bool,
